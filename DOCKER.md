...
**AutoHotkey v2 is Windows-only and cannot execute scripts inside the Docker container.** This Docker setup provides:
...
1. **No Script Execution**: AutoHotkey scripts cannot run inside the container (Windows-only)
2. **File Access**: Limited to files inside the container (use volumes for access)
