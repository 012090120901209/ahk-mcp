...
function pushUniqueFile(paths, candidate) {
    const normalized = normalizeDir(candidate);
    if (!normalized) {
        return;
    }
...
export function resolveAutoHotkeyPath(options = {}) {
...
export function getPrioritizedFileSearchPlan(options = {}) {
    const cfg = loadConfig();
    ...
    const strategy = includeConfiguredSearchDirs || includeCwdFallback ? 'expanded' : 'focused';
    const prioritized = [];
    const breakdown = {
        primary: [],
        lib: [],
    ...
    const addToBucket = (bucket, candidate) => {
        if (!isExistingDirectory(candidate)) {
            return;
        }
        const normalized = path.resolve(candidate);
...
function searchFileInDirectories(candidatePaths, directories, attemptedPaths) {
...
export function resolveFilePathDetailed(pathOrName, options = {}) {
    ...
    const focusedMatch = searchFileInDirectories(candidatePaths, focusedPlan.directories, result.attemptedPaths);
    if (focusedMatch) {
    ...
    if (includeConfiguredSearchDirs || includeCwdFallback) {
        ...
        const expandedOnlyDirectories = expandedPlan.directories.filter(dir => !result.searchDirectories.includes(dir));
        result.searchDirectories.push(...expandedOnlyDirectories);
        const expandedMatch = searchFileInDirectories(candidatePaths, expandedOnlyDirectories, result.attemptedPaths);
        if (expandedMatch) {
...
export function resolveFilePath(pathOrName, options = {}) {
