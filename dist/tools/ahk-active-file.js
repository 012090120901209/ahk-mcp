...
export class AhkActiveFileTool {
    ...
    resolveTargetPath(filePath, path) {
        if (filePath && path && filePath !== path) {
            logger.warn(`AHK_Active_File received both filePath and path with different values; using filePath. filePath="${filePath}", path="${path}"`);
            return filePath;
        }
        if (!filePath && path) {
            logger.warn('AHK_Active_File received deprecated argument "path". Use "filePath" instead for forward compatibility.');
            return path;
        }
        return filePath;
    }
